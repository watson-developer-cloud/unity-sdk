...
namespace IBM.Watson.Discovery.V2
{
    public partial class DiscoveryService : BaseService
    {
        ...
        /// format. The current version is `2020-08-30`.</param>
        ...
        public bool QueryCollectionNotices(Callback<QueryNoticesResponse> callback, string projectId, string collectionId, string filter = null, string query = null, string naturalLanguageQuery = null, long? count = null, long? offset = null)
        {
            if (callback == null)
                throw new ArgumentNullException("`callback` is required for `QueryCollectionNotices`");
            if (string.IsNullOrEmpty(Version))
                throw new ArgumentNullException("`Version` is required");
            if (string.IsNullOrEmpty(projectId))
            ...
            foreach (KeyValuePair<string, string> kvp in Common.GetSdkHeaders("discovery", "V2", "QueryCollectionNotices"))
            {
                req.Headers.Add(kvp.Key, kvp.Value);
            }
            ...
            req.OnResponse = OnQueryCollectionNoticesResponse;

            Connector.URL = GetServiceUrl() + string.Format("/v2/projects/{0}/collections/{1}/notices", projectId, collectionId);
            Authenticator.Authenticate(Connector);

            return Connector.Send(req);
        }

        private void OnQueryCollectionNoticesResponse(RESTConnector.Request req, RESTConnector.Response resp)
        {
            ...
            {
                Log.Error("DiscoveryService.OnQueryCollectionNoticesResponse()", "Exception: {0}", e.ToString());
                resp.Success = false;
            }
        ...
        /// <param name="queryId">The ID of the query used for training.</param>
        /// <returns><see cref="object" />object</returns>
        public bool DeleteTrainingQuery(Callback<object> callback, string projectId, string queryId)
        {
            if (callback == null)
                throw new ArgumentNullException("`callback` is required for `DeleteTrainingQuery`");
            if (string.IsNullOrEmpty(Version))
                throw new ArgumentNullException("`Version` is required");
            if (string.IsNullOrEmpty(projectId))
            ...
            foreach (KeyValuePair<string, string> kvp in Common.GetSdkHeaders("discovery", "V2", "DeleteTrainingQuery"))
            {
                req.Headers.Add(kvp.Key, kvp.Value);
            }
            ...
            req.OnResponse = OnDeleteTrainingQueryResponse;
        ...
        private void OnDeleteTrainingQueryResponse(RESTConnector.Request req, RESTConnector.Response resp)
        {
            DetailedResponse<object> response = new DetailedResponse<object>();
            foreach (KeyValuePair<string, string> kvp in resp.Headers)
            {
            ...
            {
                Log.Error("DiscoveryService.OnDeleteTrainingQueryResponse()", "Exception: {0}", e.ToString());
                resp.Success = false;
            }
